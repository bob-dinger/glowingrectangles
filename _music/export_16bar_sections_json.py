"""Export 8-measure sections with melody from Supabase, with chord+melody detail.

Excludes intro/outro/solo/instrumental-bridge sections. Dedupes within song
(Verse I/II/III collapse to one entry).

Output: ~/Desktop/glowinggardens_claude/_music/sixteen_bar_sections.json
        (loaded by sixteen-bar-sections.html for browsing)
"""
import os, json, re
...
OUT = '/Users/robert/Desktop/glowinggardens_claude/_music/sixteen_bar_sections.json'
...
def main():
    ...
    for song in rows:
        ...
        for i, sec in enumerate(sections):
            ...
            if not (15.9 <= n_meas <= 16.1): continue
            ...
            seen_names.add(key)
            # Compact chord/note records (only fields we need to render)
            sections_out.append({
