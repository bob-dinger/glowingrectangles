...
def main():
    ...
    blocks=defaultdict(list); seen=set()
    for a,t,hj,ug,hp in rows:
        ...
        for si,s in enumerate(secs):
            ...
            for n in notes:
                ...
                if m: dur[int(m.group(1))]+=n.get('duration',0); octs.append(n['octave'])
    ...
    for (core,focus),songs in blocks.items():
        ...
        data.append({'chords':chords,'roman':roman,'focus':focus,'note':FOCUSNOTE.get(focus,'?'),
                     'reg':domreg,'n':len(songs),'songs':sorted(songs,key=lambda s:(s['a'].lower(),s['t'].lower()))})
...
<title>Block Browser</title><style>
 ...
 #main{flex:1;overflow-y:auto;padding:18px 22px}#hd{font-size:19px;font-weight:700}#sub2{color:#888;font-size:13px;margin:2px 0 12px}
...
<div id="main"><div id="hd"></div><div id="sub2"></div><div id="body"></div></div></div>
<script>
const D=__DATA__;
...
function sel(i){document.querySelectorAll('.brow').forEach(e=>e.classList.toggle('on',+e.dataset.i===i));
 ...
 document.getElementById('sub2').textContent=`${b.n} sections · reciting the ${b.note} · usually register ${b.reg>=0?'+':''}${b.reg}`;
