...
def main():
    ...
    for bars, (nph, pb) in SCALES.items():
        ...
        data[str(bars)] = items
        print(f'  {bars}-bar: {len(items)} sections, {len(Counter(x["form"] for x in items))} distinct forms')

    open(OUT, 'w').write(PAGE.replace('__DATA__', json.dumps(data)))
    print(f'wrote {OUT}')

PAGE = r"""<!doctype html><html><head><meta charset=utf-8><title>Phrase Forms · Beatles</title>
<style>
...
function buildList(){
  const items=D[SC], forms={};
  items.forEach(x=>forms[x.form]=(forms[x.form]||0)+1);
...
function render(){
  const items=D[SC].filter(x=>FORM===null||x.form===FORM);
  document.getElementById('ttl').textContent=`${SC}-bar · ${FORM||'all forms'}`;
  document.getElementById('sub').textContent=`${items.length} sections · phrase letters track the melody · click for the piano roll`;
  document.getElementById('body').innerHTML=items.sort((a,b)=>a.form.localeCompare(b.form)||a.title.localeCompare(b.title)).map(x=>{
    const cols=phraseColors(x.form);
    const chips=x.phrases.map(p=>`<span class=pl style="background:${p.L?cols[p.L[0]]:'#ccc'}">${p.L||'·'}</span>`).join('');
    return `<div class=sec onclick="openMod('${SC}',${x.i})"><div class=ph>${chips}</div>
      <div class=t>${x.title}<span class=sub>${x.sec} · ${PCN[x.tonic]} ${x.scale}${x.stitched?' · composite':''}</span></div></div>`;
  }).join('');
}
document.querySelectorAll('.scaletog button').forEach(b=>b.onclick=()=>{
  SC=b.dataset.s;FORM=null;document.querySelectorAll('.scaletog button').forEach(x=>x.classList.toggle('on',x===b));
  buildList();render();});
...
function openMod(sc,i){const x=D[sc].find(y=>y.i===i);CUR=x;
  document.getElementById('mtitle').textContent=`${x.title} — ${x.sec}`;
  document.getElementById('msub').textContent=`${x.form} · ${x.bars} bars · ${PCN[x.tonic]} ${x.scale} · ${x.bpm||'?'}bpm`;
...
function drawRoll(b){
  ...
  const PX=Math.max(9,Math.min(20,900/beats)),H=200,CH=26,cols=phraseColors(b.form);
  ...
  b.phrases.forEach(p=>{if(!p.L)return;const L=p.L[0];
    h+=`<div class=pdiv style="left:${p.b*PX}px"></div>`;
    h+=`<div class=plab style="left:${p.b*PX+4}px;background:${cols[L]}">${p.L}</div>`;});
