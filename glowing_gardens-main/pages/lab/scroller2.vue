<template>
  ...
  <v-bottom-navigation>
    <v-btn icon @click="slide_number.value = slide_number.value - 1">
...
//   script: [
//     {
//       src: "https://d3js.org/d3.v6.min.js", // Specify the version of D3 you need
//       async: true, // Load the script asynchronously
//       defer: true, // Defer loading the script until HTML parsing is complete
//     },
//   ],
// });
...
const next_button = () => {
  ...
  //if over 2 go back
  if (slide_number.value > 1) {
    slide_number.value = 0;
  }
};
...
const create_force = () => {
  ...
  //   .force("charge", d3.forceManyBody().strength(1)) // Strength of force between nodes
  //   .force("center", d3.forceCenter(300, 300)) // Attract nodes to the center of the svg
  //   .force(
  //     "collision",
  //     d3.forceCollide().radius((d) => d.radius + 1)
  //   ) // Prevent circles from overlapping
  //   .on("tick", ticked); // Function to call for each tick of the simulation
